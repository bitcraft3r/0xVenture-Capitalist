...
const Navbar: React.FC<NavbarProps> = ({ currentUser }) => {
    const buttonClass = "rounded-md py-2 px-4 hover:cursor-pointer hover:shadow-md hover:bg-[#706960] focus:shadow-neutral-200"
    const navItems = [
    ...
    return (
        <nav className="fixed w-full bg-[#555046] text-neutral-200 z-10 shadow-md p-[1rem] flex justify-between items-center h-[4rem]">
            <div className="flex items-center">
                <Link href='/'>
                    <code className={`${buttonClass} mr-[1rem] tracking-widest`}>0xVC</code>
                </Link>
                <div className="flex">
                    {navItems.map(item => (
